...
export function BaseButton({
  ...
  return (
      <button
          aria-label={ariaLabel}
          onClick={onClick}
          className={`flex items-center justify-center p-3 rounded-default text-xs text-txt-button border-default bg-cardColor hover:bg-zinc-800 transition ${className}`}
      >
        {children}
      </button>
  );
